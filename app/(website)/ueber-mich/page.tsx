...
function NavLinks() {
  return NAVIGATION.map((item) => <a key={item.label} href={item.href} aria-current={"current" in item && item.current ? "page" : undefined}>{item.href === "/wirklich-deins/" ? <OfferName tone="light" uppercase /> : item.label}</a>);
}
    ...
    <div className={`${shell.page} ${styles.aboutPage}`}>
      ...
      <main id="inhalt">
        ...
        <section className={styles.paths}>
          <div className={styles.pathsIntro}><Marker number="07" round>WENN MEINE ART ZU ARBEITEN ZU DIR PASST</Marker><h2><span className={styles.pathsTitleLead}>Zwei eigenständige</span><span className={styles.pathsTitleRow}><span>Wege</span><em><span>der</span><span>Zusammenarbeit.</span></em></span></h2><p>Vielleicht geht es bei dir um einen konkreten Punkt, an dem du gerade feststeckst. Oder du möchtest etwas über einen längeren Zeitraum verändern, aufbauen oder verwirklichen.</p></div>
          <div className={styles.pathGrid}><article><p>01 · EIN KONKRETER PUNKT</p><h3><span>Klarheits-</span><span>sitzung</span></h3><span>Für ein konkretes Thema, bei dem du gerade feststeckst und jetzt weiterkommen willst. Du erkennst wieder, was für dich wirklich stimmig ist, und gehst mit einem nächsten Schritt aus der Sitzung, mit dem du selbstständig weitergehen kannst.</span><ActionLink href="/klarheitssitzung/" tracking={{ cta: "klarheitssitzung_kennenlernen", location: "ueber_mich_angebote" }}>ZUR KLARHEITSSITZUNG</ActionLink></article><article><p>02 · SECHS MONATE</p><h3><OfferName tone="dark" /></h3><span>Für das, was du in deinem Leben verändern, aufbauen oder verwirklichen möchtest. Über sechs Monate richtest du dich immer wieder daran aus, triffst deine Entscheidungen von dort aus und gehst konkrete Schritte, die für dich stimmig sind.</span><ActionLink href="/wirklich-deins/" light tracking={{ cta: "wirklich_deins_kennenlernen", location: "ueber_mich_angebote" }}><OfferName tone="dark" uppercase /> ENTDECKEN</ActionLink></article></div>
        </section>
      </main>

      <footer className={`${shell.footer} ${siteStyles.siteFooter}`}><a className={shell.footerBrand} href="#top"><img src="/petra-sailer-footer-logo.svg" alt="" /></a><nav aria-label="Footer-Navigation"><a href="#top">ÜBER MICH</a><a href="/klarheitssitzung/">KLARHEITSSITZUNG</a><a href="/wirklich-deins/"><OfferName tone="dark" /></a><a href="/kontakt/">KONTAKT</a><a href="/arbeite-mit-mir/">ARBEITE MIT MIR</a></nav><a className={shell.footerEmail} href="mailto:kontakt@petrasailer.com" data-umami-event="contact_click" data-umami-event-method="email" data-umami-event-location="footer">kontakt@petrasailer.com</a><div className={shell.footerBottom}><p>© Petra Sailer 2026</p><div><a href="https://petrasailer.com/impressum/">Impressum</a><a href="https://petrasailer.com/datenschutz/">Datenschutz</a></div></div></footer>
    </div>
  );
}
